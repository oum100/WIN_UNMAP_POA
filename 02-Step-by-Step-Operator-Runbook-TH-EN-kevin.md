...
```text
Use 03-TrimLatencyTrace-kevin.ps1 instead of 03-TrimLatencyTrace.ps1.
This Kevin version automatically captures:
  - ScheduledDefrag task configuration by schtasks /query
  - Win32_Process details every 60 seconds for suspected optimizer activity
```
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Start -OutputDir $OutputDir -SampleInterval 5
...
TrimTest-KevinProcessCim Running

disk_latency_perfmon.blg created under <OutputDir>
kevin-scheduleddefrag-task-config.txt created under <OutputDir>
kevin-process-cim-monitor.csv created under <OutputDir>
```
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Mark -OutputDir $OutputDir -Message "Before delete SQL backup"
```
...
```powershell
Remove-Item "E:\Backup\LargeSqlBackup.bak" -Force
```
...
```text
Replace E:\Backup\LargeSqlBackup.bak with the actual file path.
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Mark -OutputDir $OutputDir -Message "After delete SQL backup"
```
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Status -OutputDir $OutputDir
```
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Stop -OutputDir $OutputDir
...
kevin-scheduleddefrag-task-config.txt
kevin-process-cim-monitor.csv
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Mark -OutputDir $OutputDir -Message "Before manual Optimize-Volume ReTrim"
```
...
```text
Replace drive letter E with the actual target volume drive letter.
...
.\03-TrimLatencyTrace-kevin.ps1 -Action Mark -OutputDir $OutputDir -Message "After manual Optimize-Volume ReTrim"
```
